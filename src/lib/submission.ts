...
class Submission {
    ...
    async detail(): Promise<Submission> {
        ...
        this.totalTC = response.match(/total_testcases :\s'([^']*)'/)[1];
        this.correctTC = response.match(/total_correct :\s'([^']*)'/)[1];
        this.input = JSON.parse(`"${response.match(/input :\s'([^']*)'/)[1]}"`);
        this.output = response.match(/code_output :\s'([^']*)'/)[1];
        this.expected = response.match(/expected_output :\s'([^']*)'/)[1];
